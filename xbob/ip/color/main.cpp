...
static xbob::extension::FunctionDoc s_rgb_to_gray = xbob::extension::FunctionDoc(
    ...
    "Converts an RGB color-coded pixel or a full array to Grayscale",
    ...
    .add_prototype("r, g, b", "scalar")
    .add_parameter("input", "array_like (uint8|uint16|float64, 3D)", "Input array containing an image with the shape ``(3, height, width)``")
    .add_parameter("output", "array_like (uint8|uint16|float64, 2D), optional", "Output array - if provided, should have matching data type to ``input``. The shape should be ``(height, width)``")
    ...
    .add_return("scalar", "scalar (uint8|uint16|float64)", "A scalar is returned when this function is fed discrete RGB values. The type matches the input pixel values")
;
