...
/**
static object hsv_to_rgb(const object& y, const object& dtype){
  bob::python::dtype d(dtype);
  switch (d.eltype()) {
    case bob::core::array::t_uint8:
      {
        uint8_t r, g, b;
        bob::ip::hsv_to_rgb_one((uint8_t)extract<uint8_t>(y), r, g, b);
        return make_tuple(r, g, b);
      }
    case bob::core::array::t_uint16:
      {
        uint16_t r, g, b;
        bob::ip::hsv_to_rgb_one((uint16_t)extract<uint8_t>(y), r, g, b);
        return make_tuple(r, g, b);
      }
    case bob::core::array::t_float64:
      {
        double r, g, b;
        bob::ip::hsv_to_rgb_one((double)extract<uint8_t>(y), r, g, b);
        return make_tuple(r, g, b);
      }
    default:
...
static object rgb_to_hsv(const object& r, const object& g, const object& b, const object& dtype){
  ...
  switch (d.eltype()) {
    case bob::core::array::t_uint8:
      {
        uint8_t y, u, v;
        bob::ip::rgb_to_hsv_one((uint8_t)extract<uint8_t>(r), (uint8_t)extract<uint8_t>(g), (uint8_t)extract<uint8_t>(b), y, u, v);
        return make_tuple(y, u, v);
      }
    case bob::core::array::t_uint16:
      {
        uint16_t y, u, v;
        bob::ip::rgb_to_hsv_one((uint16_t)extract<uint16_t>(r), (uint16_t)extract<uint16_t>(g), (uint16_t)extract<uint16_t>(b), y, u, v);
        return make_tuple(y, u, v);
      }
    case bob::core::array::t_float64:
      {
        double y, u, v;
        bob::ip::rgb_to_hsv_one((double)extract<double>(r), (double)extract<double>(g), (double)extract<double>(b), y, u, v);
        return make_tuple(y, u, v);
      }
    default:
      PYTHON_ERROR(TypeError,
...
static object hsv_to_rgb(const object& y, const object& u, const object& v, const object& dtype){
  bob::python::dtype d(dtype);
  switch (d.eltype()) {
    case bob::core::array::t_uint8:
      {
        uint8_t r, g, b;
        bob::ip::hsv_to_rgb_one((uint8_t)extract<uint8_t>(y), (uint8_t)extract<uint8_t>(u), (uint8_t)extract<uint8_t>(v), r, g, b);
        return make_tuple(r, g, b);
      }
    case bob::core::array::t_uint16:
      {
        uint16_t r, g, b;
        bob::ip::hsv_to_rgb_one((uint16_t)extract<uint16_t>(y), (uint16_t)extract<uint16_t>(u), (uint16_t)extract<uint16_t>(v), r, g, b);
        return make_tuple(r, g, b);
      }
    case bob::core::array::t_float64:
      {
        double r, g, b;
        bob::ip::hsv_to_rgb_one((double)extract<double>(y), (double)extract<double>(u), (double)extract<double>(v), r, g, b);
        return make_tuple(r, g, b);
      }
    default:
      PYTHON_ERROR(TypeError,
...
void py_hsv_to_rgb (bob::python::const_ndarray from, bob::python::ndarray to)
{
  switch (from.type().dtype) {
    case bob::core::array::t_uint8:
      {
        blitz::Array<uint8_t,3> to_ = to.bz<uint8_t,3>();
        bob::ip::hsv_to_rgb(from.bz<uint8_t,2>(), to_);
      }
      break;
    case bob::core::array::t_uint16:
      {
        blitz::Array<uint16_t,3> to_ = to.bz<uint16_t,3>();
        bob::ip::hsv_to_rgb(from.bz<uint16_t,2>(), to_);
      }
      break;
    case bob::core::array::t_float64:
      {
        blitz::Array<double,3> to_ = to.bz<double,3>();
        bob::ip::hsv_to_rgb(from.bz<double,2>(), to_);
      }
      break;
    default:
      PYTHON_ERROR(TypeError,
...
static const char* rgb_to_hsv_doc = "Takes a 3-dimensional array encoded as RGB and sets the second array with HSV (Y'CbCr) equivalents as determined by rgb_to_hsv_one(). The array must be organized in such a way that the color bands are represented by the first dimension. Its shape should be something like (3, width, height) or (3, height, width). The output array has to have the required size for the conversion otherwise an exception is raised (except for versions allocating the returned arrays). WARNING: As of this time only C-style storage arrays are supported.";
static const char* hsv_to_rgb_doc = "Takes a 3-dimensional array encoded as HSV (Y'CbCr) and sets the second array with RGB equivalents as determined by hsv_to_rgb_one(). The array must be organized in such a way that the color bands are represented by the first dimension.  Its shape should be something like (3, width, height) or (3, height, width). The output array has to have the required size for the conversion otherwise an exception is raised (except for versions allocating the returned arrays). WARNING: As of this time only C-style storage arrays are supported.";
...
static const char* hsv_to_rgb_doc = "Takes a 2-dimensional array encoded as hsvs and sets the second array with RGB equivalents as determined by hsv_to_rgb_one(). The output array has to have the required size for the conversion otherwise an exception is raised (except for versions allocating the returned arrays). WARNING: As of this time only C-style storage arrays are supported";
...
void bind_ip_color()
{
  ...
  def("rgb_to_hsv", &rgb_to_hsv, (arg("red"), arg("green"), arg("blue"), arg("dtype")), "Converts a RGB color-coded pixel to HSV (Y'CbCr) using the CCIR 601 (Kb = 0.114, Kr = 0.299) norm as discussed here: http://en.wikipedia.org/wiki/YCbCr and here: http://www.fourcc.org/fccyvrgb.php. Returns a tuple with (y,u,v) values.\n\n Depending on the dtype parameter, different types of data is expected:\n\n - 'float': float values between 0 and 1\n - 'uint8': integers between 0 and 255\n - 'uint16': integers between 0 and 65535");
  def("hsv_to_rgb", &hsv_to_rgb, (arg("y"), arg("u"), arg("v"), arg("dtype")), "Converts a HSV (Y'CbCr) color-coded pixel to RGB using the CCIR 601 (Kb = 0.114, Kr = 0.299) norm as discussed here: http://en.wikipedia.org/wiki/YCbCr and here: http://www.fourcc.org/fccyvrgb.php. Returns a tuple with (r,g,b) values.\n\n Depending on the dtype parameter, different types of data is expected:\n\n - 'float': float values between 0 and 1\n - 'uint8': integers between 0 and 255\n - 'uint16': integers between 0 and 65535");
  def("rgb_to_hsv", &rgb_to_hsv, (arg("red"), arg("green"), arg("blue"), arg("dtype")), "Converts a RGB color-coded pixel to Hsvscale using the CCIR 601 (Kb = 0.114, Kr = 0.299) norm as discussed here: http://www.fourcc.org/fccyvrgb.php. Returns only the hsv value (Y component) in the desired data format. This method is more efficient than calling rgb_to_hsv() method just to extract the Y component.\n\n Depending on the dtype parameter, different types of data is expected:\n - 'float': float values between 0 and 1\n - 'uint8': integers between 0 and 255\n - 'uint16': integers between 0 and 65535");
  def("hsv_to_rgb", &hsv_to_rgb, (arg("y"), arg("dtype")), "Converts a hsvscale pixel to RGB by copying the hsv value to all 3 bands. Returns a tuple with (r,g,b) values. This method is just here for convenience.\n Depending on the dtype parameter, different types of data is expected:\n\n - 'float': float values between 0 and 1\n - 'uint8': integers between 0 and 255\n - 'uint16': integers between 0 and 65535");
